...
err_logs_fp = open('err_logs.txt','a+')
...
def load_frame_lis(vid_pth):
    ...
    if os.path.isdir(vid_pth) : 
        ...
        try :
            ...
            err_logs_fp.writelines('err1 when reading : {} \n'.format(vid_pth))
            err_logs_fp.flush()
            return None
...
def pro_vid_1(vp,name_mode,bool_only96):
    # print(vp.split('_'))
    ...
    gt_vp = get_pth_gt2( word , vid )
    # gt_vp = get_pth_gt2( pid , emo , lev , vid )
...
if __name__ == '__main__':
    ...
    with open('result_lrw/{}.txt'.format(save_name),'w') as f:
        for vp in vlis:
            mouth_lmd , mouth_lvd , face_lmd , face_lvd = pro_vid_1(vp,name_mode,bool_only96)
            if mouth_lmd != -1 : 
